...
def get_detail_value(page, label):
    row = page.locator(f'#prodDetails tr:has(th:has-text("{label}"))')

    if row.count() == 0:
        row = page.locator(f'#features_and_specs tr:has(th:has-text("{label}"))')

    if row.count() == 0:
        row = page.locator(f'#item_details tr:has(th:has-text("{label}"))')
...
def scrape_mattress_best_sellers():
    with sync_playwright() as p:
        ...
        all_asins = []
        bsr_base_url = (
            'https://www.amazon.com/Best-Sellers-Home-Kitchen-Mattresses/zgbs/home-garden/3732981/'
        )
        # 爬取前 2 页，共 100 个产品
        for pg in [1, 2]:
        ...
        for index, asin in enumerate(all_asins):
            ...
            try:
                ...
                main_name, main_rank, sub_name, sub_rank = get_detail_value(
                    page, 'Best Sellers Rank'
                )
                ...
                print(f' [x] ASIN {asin} 抓取失败: {e!s}')
                current_app.logger.error(f' [x] ASIN {asin} 抓取失败: {e!s}')

            # 详情页之间必须有随机停顿，否则必封 IP
            sleep_time = random.choice(
                [random.uniform(2, 5), random.uniform(5, 8), random.uniform(8, 12)]
            )
